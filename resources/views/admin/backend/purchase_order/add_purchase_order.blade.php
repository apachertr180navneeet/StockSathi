    ...
    <div class="container-xxl">
        <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
            <div class="flex-grow-1">
                <h4 class="fs-18 fw-semibold m-0">{{ isset($requisition) ? 'Convert Requisition to Purchase Order' : 'Add Purchase Order' }}</h4>
            </div>
            <div class="text-end">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('store.purchase.order') }}" method="post" id="po_form">
                            @csrf
                            ...
                            <div class="row">
                                <div class="col-md-3 mb-3">
                                    ...
                                    <input type="date" name="date" class="form-control" value="{{ isset($requisition) ? $requisition->date : date('Y-m-d') }}" required>
                                </div>
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select name="supplier_id" id="supplier_id" class="form-control form-select" required>
                                        ...
                                        @foreach($suppliers as $sup)
                                            <option value="{{ $sup->id }}" {{ (isset($requisition) && $requisition->supplier_id == $sup->id) ? 'selected' : '' }}>{{ $sup->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-3 mb-3">
                                    ...
                                    <select name="warehouse_id" id="warehouse_id" class="form-control form-select" required>
                                        ...
                                        @foreach($warehouses as $wh)
                                            <option value="{{ $wh->id }}" {{ (isset($requisition) && $requisition->warehouse_id == $wh->id) ? 'selected' : '' }}>{{ $wh->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                        ...
                                        @if(isset($requisition))
                                            @foreach($requisition->requisitionItems as $item)
                                                <tr data-id="{{ $item->product_id }}">
                                                    ...
                                                    <td>
                                                        <input type="number" step="0.01" name="products[{{ $item->product_id }}][unit_cost]" class="form-control unit_cost" value="{{ $item->estimated_cost }}" required>
                                                    </td>
                                                    <td>
                                                        <input type="number" name="products[{{ $item->product_id }}][quantity]" class="form-control quantity" value="{{ $item->quantity }}" required>
                                                    </td>
                                                    <td>
                                                        <input type="number" step="0.01" name="products[{{ $item->product_id }}][tax_rate]" class="form-control tax_rate" value="0">
                                                        <input type="hidden" name="products[{{ $item->product_id }}][tax_amount]" class="tax_amount" value="0">
                                                    </td>
                                                    <td>
                                                        <input type="number" step="0.01" name="products[{{ $item->product_id }}][discount]" class="form-control discount" value="0">
                                                    </td>
                                                    <td>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <textarea name="note" class="form-control" rows="4">{{ isset($requisition) ? 'Converted from Requisition: ' . $requisition->requisition_no : '' }}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-6 text-end">
                                    <table class="table table-borderless">
                                        <tr>
                                            <th>Order Tax (%)</th>
                                            <td><input type="number" step="0.01" name="tax_rate" id="order_tax_rate" class="form-control text-end" value="0"></td>
                                        </tr>
                                        <tr>
                                            <th>Order Discount</th>
                                            <td><input type="number" step="0.01" name="discount" id="order_discount" class="form-control text-end" value="0"></td>
                                        </tr>
                                        <tr>
                                            <th>Shipping</th>
                                            <td><input type="number" step="0.01" name="shipping" id="order_shipping" class="form-control text-end" value="0"></td>
                                        </tr>
