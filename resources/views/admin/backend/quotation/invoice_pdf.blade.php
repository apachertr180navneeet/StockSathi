...
<head>
    ...
    <style>
        ...
        .status-badge.Accepted { background-color: #17a2b8; }
    </style>
</head>
<body>
    <div class="invoice-container">
        <div class="header">
            ...
            <div class="header-right">
                ...
                <div class="invoice-details">
                    <p><strong>Quotation No:</strong> #QUOTE-{{ str_pad($quotation->id, 5, '0', STR_PAD_LEFT) }}</p>
                    <p><strong>Date:</strong> {{ date('d M Y', strtotime($quotation->date)) }}</p>
                    <p><strong>Status:</strong> <span class="status-badge {{ $quotation->status }}">{{ $quotation->status }}</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="billing-info">
            <div class="bill-to">
                ...
                <p style="margin: 5px 0 2px; font-weight: bold;">{{ $quotation->customer->name }}</p>
                @if($quotation->customer->email)<p style="margin: 0 0 2px;">{{ $quotation->customer->email }}</p>@endif
                @if($quotation->customer->phone)<p style="margin: 0 0 2px;">{{ $quotation->customer->phone }}</p>@endif
                @if($quotation->customer->address)<p style="margin: 0 0 2px;">{{ $quotation->customer->address }}</p>@endif
            </div>
            <div class="ship-from">
                <div class="section-title" style="text-align: right;">Dispatched From</div>
                <p style="margin: 5px 0 2px; font-weight: bold;">{{ $quotation->warehouse->name }}</p>
                @if($quotation->warehouse->phone)<p style="margin: 0 0 2px;">Tel: {{ $quotation->warehouse->phone }}</p>@endif
                @if($quotation->warehouse->email)<p style="margin: 0 0 2px;">{{ $quotation->warehouse->email }}</p>@endif
                @if($quotation->warehouse->address)<p style="margin: 0 0 2px;">{{ $quotation->warehouse->address }}</p>@endif
            </div>
        </div>
        ...
        <table class="items">
            ...
            <tbody>
                @foreach ($quotation->quotationItems as $key => $item)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>
        ...
        <div class="totals-section">
            <div class="totals-space">
                ...
                <div style="margin-top: 20px; padding: 10px; background: #f9f9f9; border-left: 3px solid #0056b3;">
                    ...
                    <p style="margin: 0; font-size: 11px;">{{ $quotation->note }}</p>
                </div>
                @endif
            </div>
            <div class="totals">
                @php
                    $subtotal = $quotation->quotationItems->sum('subtotal');
                @endphp
                <table>
                    <tr>
                        <td>Subtotal</td>
                    ...
                    <tr>
                        ...
                        <td class="text-right" style="color: #dc3545;">-₹{{ number_format($quotation->discount, 2) }}</td>
                    </tr>
                    @endif
                    <tr>
                        <td>Tax (GST @ {{ number_format($quotation->tax_rate, 2) }}%)</td>
                        <td class="text-right">₹{{ number_format($quotation->tax_amount, 2) }}</td>
                    </tr>
                    @if($quotation->shipping > 0)
                    <tr>
                        <td>Shipping Cost</td>
                        <td class="text-right">₹{{ number_format($quotation->shipping, 2) }}</td>
                    </tr>
                    @endif
